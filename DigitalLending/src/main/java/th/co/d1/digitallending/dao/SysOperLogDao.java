...
public class SysOperLogDao {
    ...
    final static Logger logger = Logger.getLogger(SysLookupDao.class);
        ...
        try {
            ...
            if (null != sysLog) {
                ...
                if ((null == list || list.isEmpty()) && null != sysOperLog.getCaseId() && !"".equals(sysOperLog.getCaseId())) {
                    ...
                    if (sysOperLog.getStatus() == StatusUtils.getPass(dbEnv).getStatusCode()) {
...
//                JSONObject summaryDtl = new JSONObject();
                while (rs.next()) {
                    ...
                    if (logObj.has(DateUtils.getDisplayEnDate(sysOperLog.getBusinessDate(), "dd-MM-yyyy"))) {
                        ...
                        psOperLog.setInt(2, Integer.parseInt(sysOperLog.getProductVersionId()));
                        stepDataRs = psOperLog.executeQuery();
                        while (stepDataRs.next()) {
        ...
        if ((null != startDate && !startDate.isEmpty()) && (null != endDate && !endDate.isEmpty())) {
            try {
                ...
                while (sDate.compareTo(eDate) <= 0) {
                    ret = queryReconcileReport(con, prodCode, sdf.format(sDate), ret, "");
            ...
            ret = queryReconcileReport(con, prodCode, endDate, ret, "end");
        } else if ((null != startDate && !startDate.isEmpty()) && (null == endDate || endDate.isEmpty())) {
            ret = queryReconcileReport(con, prodCode, startDate, ret, "start");
        }
        return ret;
    }

    public JSONObject getTransactionReport(String dbEnv, String prodCode, String txnId, String refNo, String ucId, String paymentMethod, String startDate, String endDate, String startTime, String endTime, Integer status, String state, String refTxnId) {
        Connection con = null;
...
//            JSONArray prodscutoff = getProductCutoffTime(con, prodCode);
            /*
[{
        "prodver": "1",
        "activeDate": "2020-04-01",
        "expireDate": "2020-12-20",
        "prodid": "e0c80f8b-627e-48b4-9384-87d00dca5274",
        "cutoff": "All"
    }, {
        "prodver": "0",
        ...
        "cutoff": "10:00"
             ...
             */
//            System.out.println(prodscutoff);
...
//                transactionCmd.append(" AND LOG.ATTR1 = ? ");
            ...
            try {
                ps = con.prepareStatement(transactionCmd.toString());
                if (params.size() > 0) {
                    for (int i = 0; i < params.size(); i++) {
...
//                    transactionCmd.append("SELECT STEP_DATA, PAYMENT_METHOD FROM T_SYS_OPER_LOG WHERE TXN_NO = ? AND STATE_CODE = '71228c52-4322-11ea-b77f-2e728ce88125'"); //PRO1011
                    ...
                    psOperLog = con.prepareStatement(transactionCmd.toString());
                    psOperLog.setString(1, sysOperLog.getTxnNo());
                    stepDataRs = psOperLog.executeQuery();
                    JSONObject stepData = new JSONObject();
                    stepData.put("paymentMethod", sysOperLog.getPaymentMethod());
...
//            rs.close();
//            ps.close();
        } catch (SQLException | HibernateException | NullPointerException | ParseException e) {
            logger.error("" + e);
            e.printStackTrace();
        } finally {
            try {
...
//                transactionCmd.append(" AND LOG.ATTR1 = ? ");
            ...
            try {
                ps = con.prepareStatement(inquiryCmd.toString());
                if (params.size() > 0) {
                    for (int i = 0; i < params.size(); i++) {
                        if (params.get(i) instanceof String) {
                            ps.setString(i + 1, (String) params.get(i));
...
//                    inquiryCmd.append("SELECT STEP_DATA, PAYMENT_METHOD FROM T_SYS_OPER_LOG WHERE TXN_NO = ? AND STATE_CODE = '71228c52-4322-11ea-b77f-2e728ce88125'");
                    inquiryCmd.append("SELECT STEP_DATA, PAYMENT_METHOD FROM T_SYS_OPER_LOG WHERE TXN_NO = ? AND STATE_CODE = (SELECT UUID FROM T_SHELF_LOOKUP WHERE GROUP_TYPE = 'PROCESS_STATE' AND LOOKUP_CODE = 'PRO1011')"); //PRO1011
                    psOperLog = con.prepareStatement(inquiryCmd.toString());
                    psOperLog.setString(1, sysOperLog.getTxnNo());
                    stepDataRs = psOperLog.executeQuery();
                    JSONObject stepData = new JSONObject();
                    stepData.put("paymentMethod", sysOperLog.getPaymentMethod());
...
//                }
    ...
    public JSONArray getErrorLogList(String dbEnv, JSONObject param) {
        Connection con = null;
...
//            }
            ...
            ps = con.prepareStatement(errorLogCmd.toString());
            if (params.size() > 0) {
                for (int i = 0; i < params.size(); i++) {
                    if (params.get(i) instanceof String) {
                        ps.setString(i + 1, (String) params.get(i));
...
//                }
    ...
    public JSONArray getListTaskCategory(String dbEnv) {
        Connection con = null;
        JSONArray arr = new JSONArray();
        PreparedStatement ps = null;
        ResultSet rs = null;
...
//            ps.close();
    ...
    private JSONArray queryReconcileReport(Connection con, String prodCode, String businessDate, JSONArray ret, String dateType) {
//        JSONArray ret = new JSONArray();
        PreparedStatement ps = null, psOperLog = null;
        ResultSet rs = null, stepDataRs = null;
        String cutOff = "";
                ...
                if (rs.next() == false) {
                    ...
                    if (null != prodCode && !prodCode.isEmpty()) {
                        ...
                        productDetail.append("SELECT LK_CODE,LK_LABEL,LK_VALUE FROM T_SHELF_PRODUCT_DTL WHERE 1=1 "
                                ...
                                + "AND LK_CODE IN ('cutOffTime','pcutOffDay','pcutOffWE','pcutOffTime','pcutOffTAllD','pcutOffTSpec', 'prodName', 'prodType')");
                        psOperLog = con.prepareStatement(productDetail.toString());
...
//                }
    ...
    private JSONArray getProductCutoffTime(Connection con, String prodid) {
        JSONArray arr = new JSONArray();
        ResultSet rs = null;
        PreparedStatement ps = null;
        try {
            ...
            ps = con.prepareStatement(dashBoardCmd.toString());
