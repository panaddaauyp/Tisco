        ...
        try (Session session = getSessionMaster(dbEnv).openSession()) {
            ...
            while (rs.next()) {
                JSONObject obj = new JSONObject().put("uuid", ValidUtils.null2NoData(rs.getString("UUID")))
                        .put("productCode", ValidUtils.null2NoData(rs.getString("PROD_CODE")))
                        .put("productName", ValidUtils.null2NoData(rs.getString("PROD_NAME")));
                ret.put(obj);
            }
        } catch (HibernateException | NullPointerException e) {
            logger.info(e.getMessage());
        ...
        try (Session session = getSessionMaster(dbEnv).openSession()) {
            ...
            if (department == null || department == "") {
                ...
                if (null != department && !department.isEmpty()) {
                    searchProdCmd.append("AND LK_VALUE = ? ");
                    params.add(department);
                }
                searchProdCmd.append(") AND STATUS = 213 )");
            }
            ps = session.doReturningWork((Connection conn) -> conn).prepareStatement(searchProdCmd.toString());
            if (params.size() > 0) {
                for (int i = 0; i < params.size(); i++) {
...
//                arr.put(data);
    ...
    public JSONArray getListMapProductStatus(String dbEnv,String product) throws SQLException{
        ...
        try (Session session = getSessionMaster(dbEnv).openSession()) {
            ...
            while (rs.next()) {
                JSONObject obj = new JSONObject().put("uuid", ValidUtils.null2NoData(rs.getString("UUID")))
                        .put("lookupCode", ValidUtils.null2NoData(rs.getString("LOOKUP_CODE")))
                        .put("lookupNameTh", ValidUtils.null2NoData(rs.getString("LOOKUP_NAME_TH")))
                        .put("lookupNameEn", ValidUtils.null2NoData(rs.getString("LOOKUP_NAME_EN")))
                        .put("lookupValue", ValidUtils.null2NoData(rs.getString("LOOKUP_VALUE")));
                ret.put(obj);
            }
        } catch (HibernateException | NullPointerException e) {
            logger.info(e.getMessage());
    ...
    public JSONArray getListMapStatusComponent(String dbEnv,String status) throws SQLException{
        ...
        try (Session session = getSessionMaster(dbEnv).openSession()) {
            ...
            if (null == status || status.isEmpty()) {
                ...
                cmd.append(" ) ORDER BY SEQ_NO ASC ");
            }
            ps = session.doReturningWork((Connection conn) -> conn).prepareStatement(cmd.toString());
            if (params.size() > 0) {
                for (int i = 0; i < params.size(); i++) {
    ...
    public JSONArray getListStateByCompStatus(String dbEnv, String component, String status) throws SQLException{
        ...
        try (Session session = getSessionMaster(dbEnv).openSession()) {
            ...
            if (component == "" || status == "") {
                ...
                if (null != status && !status.isEmpty()) {
                    cmd.append("AND MSC.COMPONENT_UUID = ? ");
                    params.add(status);
                }
                cmd.append(" ) ");
            }
